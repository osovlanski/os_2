...
void test7()
{
    ...
    if ((childpid = fork()) == 0)
    {
        ...
        //failedTest(7, "child : child should not reach here, it shold remain stopped\n");
    }
    else //parent code
    {
        sleep(20);
        ...
        if (!flag)
        {
            failedTest(7, "father: child should not send message, sigprocmask probably failed");
        }
...
{ //test sigprocpask with user signals  + sanity check
    ...
    if ((childpid = fork()) == 0)
    {
        ...
        kill(fatherpid, 6); //notify parent for blocking handler #3
                            //in father flag should be 0, in child flag sohuld be 0
        sleep(50);          //wait for father to change flag
        while (!flag)
        {
